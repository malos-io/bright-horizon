<template>
  <div class="page">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <div class="search-box">
          <input v-model="search" type="text" placeholder="Search institution..." class="search-input" />
        </div>
    ...
    <div v-if="loading" class="loading-state">Loading TVIs...</div>
    ...
    <div v-else class="empty-state">
      {{ search || courseFilter || activeFilter || contactedFilter ? 'No TVIs matching filters' : 'No TVIs found. Try syncing data.' }}
    </div>

    <!-- Edit Modal -->
    <div v-if="editing" class="modal-overlay" @click.self="editing = null">
      <div class="modal">
        <h3>{{ editing.school }}</h3>
        ...
        <div class="toggle-row">
          <input type="checkbox" v-model="editForm.contacted" id="tvi-contacted-toggle" />
          <label for="tvi-contacted-toggle">{{ editForm.contacted ? 'Yes' : 'No' }}</label>
        </div>
...
async function saveEdit() {
  saving.value = true
  try {
    await updateTesdaTvi(editing.value.id, editForm.value)
    Object.assign(editing.value, editForm.value)
    editing.value = null
  } catch (err) {
    console.error('Failed to update TVI:', err)
  } finally {
    saving.value = false
  }
}
...
async function loadData() {
  ...
  try {
    ...
    console.error('Failed to load TVIs:', err)
  } finally {
    loading.value = false
  }
}
